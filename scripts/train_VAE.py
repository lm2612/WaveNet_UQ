...
model_filename = f"{model_architecture}_model.pth"
path_to_model = f"{save_dir}{model_filename}"
weights_filename = f"{model_architecture}_weights.pth"
path_to_weights = f"{save_dir}{weights_filename}"
losses_filename = f"{model_architecture}_losses.csv"
path_to_losses = f"{save_dir}{losses_filename}"
training_losses_filename = f"{model_architecture}_training_losses.csv"
path_to_training_losses = f"{save_dir}{training_losses_filename}"
validation_losses_filename = f"{model_architecture}_validation_losses.csv"
...
if init_epoch==0:
    ...
    my_model = VAE(n_in=82, n_out=n_out)
    losses =[]
    training_losses = []
    validation_losses = []
    ...
    weights_filename = f"{model_architecture}_weights_epoch{init_epoch}.pth"
    path_to_weights = f"{save_dir}{weights_filename}"
...
for ep in range(init_epoch+1, n_epochs):
    ...
    # Save model at end of each epoch
    weights_filename = f"{model_architecture}_weights_epoch{ep}.pth"
    path_to_weights = f"{save_dir}{weights_filename}"
